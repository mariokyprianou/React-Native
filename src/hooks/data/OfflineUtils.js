...
async function getWorkoutPayloads() {
    let existing = (await AsyncStorage.getItem(WORKOUT_COMPLETE_PAYLOAD).catch((error) => console.log(LOG, error))) || '[]';
    existing = JSON.parse(existing);
    return existing;
}

async function getFirebasePayloads() {
    let existingLogs = (await AsyncStorage.getItem(WORKOUT_COMPLETE_FIREBASE_PAYLOAD).catch((error) => console.log(LOG, error))) || '[]';
    existingLogs = JSON.parse(existingLogs);
    return existingLogs;
}

async function getWorkoutsCompleteIncreament() {
    let existingIncrement = await AsyncStorage.getItem(WORKOUTS_COMPLETED).catch((error) => console.log(LOG, error));
...
async function completeWorkout(workoutComplete, firebaseEventPayload) {
    ...
    await AsyncStorage.setItem(WORKOUT_COMPLETE_PAYLOAD, JSON.stringify(existing)).catch((error) => console.log(LOG, error));
    ...
    await AsyncStorage.setItem(WORKOUT_COMPLETE_FIREBASE_PAYLOAD, JSON.stringify(existingLogs)).catch((error) => console.log(LOG, error));
    ...
    await AsyncStorage.setItem(WORKOUTS_COMPLETED, JSON.stringify(existingIncrement)).catch((error) => console.log(LOG, error));
}
...
// When offline 
// todo -- getProgramme -> add workouts completed from the asyncstorage ones
// todo -- getProfile -> add increament of total workouts
