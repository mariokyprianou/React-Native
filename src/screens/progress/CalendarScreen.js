...
export default function CalendarScreen() {
  ...
  useEffect(() => {
    if (progress && progress.length > 0) {
      ...
      // earliest month
      let earliestMonth = completeProgress.sort(
        (a, b) => parseISO(a.startOfMonth) - parseISO(b.startOfMonth),
      )[0];
      ...
      for (let i = 1; i <= checkMonths; i++) {
        ...
        if (
          !completeProgress.find((it) =>
            sameMonth(parseISO(it.startOfMonth), newMonth),
          )
        ) {
          const firstOfMonth = newMonth;
      ...
      // Sort months again
      completeProgress = completeProgress.sort(
        (a, b) => parseISO(b.startOfMonth) - parseISO(a.startOfMonth),
      );
